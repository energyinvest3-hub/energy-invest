-- EnergyInvest / PushinPay v3
-- Corrige conciliação por gateway_id de forma case-insensitive.
-- NÃO credita nada sozinho; apenas torna o casamento de IDs robusto.
begin;

update public.deposits
set gateway_id = upper(trim(gateway_id)),
    updated_at = now()
where provider = 'pushinpay'
  and gateway_id is not null
  and gateway_id <> upper(trim(gateway_id));

create unique index if not exists deposits_pushinpay_gateway_ci_unique
  on public.deposits ((upper(gateway_id)))
...
begin
  ...
  if nullif(trim(coalesce(p_qr_code, '')), '') is null then
    raise exception 'Missing PIX code';
  end if;
  if p_value_cents is null or p_value_cents < 50 then
    raise exception 'Invalid PIX value';
  end if;
  ...
  update public.deposits
  set gateway_id = upper(trim(p_gateway_id)),
      pix_code = p_qr_code,
...
begin
  ...
  from public.deposits
  where provider = 'pushinpay'
    and upper(trim(gateway_id)) = upper(trim(p_gateway_id))
  for update;
  ...
  if v_status = 'paid' then
    if v_deposit.status = 'completed' then
      ...
      set provider_status = 'paid',
          gateway_id = upper(trim(gateway_id)),
